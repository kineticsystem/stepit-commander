...
| Package | Rule |
|---|---|
| `commander_objectives` | XML only, no code. Objectives, the subtrees they reuse and the generated node models, all in `objectives/`. |
...
`commander_behaviors::registerNodes` (`src/register_nodes.cpp`) and, for editors, a regenerated
...
**Node models for editors.** `commander_objectives/objectives/stepit_behaviors.xml` is the
`<TreeNodesModel>` of every behavior, generated by `commander_behaviors::nodesModel()` from the
...
type. The server does not read it: `commander_objectives/CMakeLists.txt` excludes it from the
