...
real registration. Editors (Groot2, the behavior editor) read it because they cannot load the
...
install, so it never reaches the installed folder listed in `behavior_trees`. `test_nodes_model` fails when it is out of date; regenerate it with
